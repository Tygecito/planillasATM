...
@section('title', 'Registrar Subsidio - Mi App')

@section('content')
    <h1 class="welcome-message">Registrar Nuevo Subsidio</h1>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('subsidios.store') }}" method="POST" id="create-subsidio-form">
                @csrf
                ...
                <div class="form-grid-empleado">
                    ...
                    <div class="form-group">
                        ...
                        <select name="tipo_subsidio" id="tipo_subsidio" class="form-control @error('tipo_subsidio') is-invalid @enderror" required>
                            ...
                            <option value="PRENATAL" {{ old('tipo_subsidio') == 'PRENATAL' ? 'selected' : '' }}>PRENATAL</option>
                            <option value="NATALIDAD" {{ old('tipo_subsidio') == 'NATALIDAD' ? 'selected' : '' }}>NATALIDAD</option>
                            <option value="LACTANCIA" {{ old('tipo_subsidio') == 'LACTANCIA' ? 'selected' : '' }}>LACTANCIA</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="monto" id="monto" class="form-control @error('monto') is-invalid @enderror" 
                               value="{{ old('monto') }}" required step="0.01" min="2500" placeholder="Mínimo 2500.00">
                    ...
                    <div class="form-group">
                        ...
                        <select name="estado" id="estado" class="form-control @error('estado') is-invalid @enderror" required>
                            <option value="POR PAGAR" {{ old('estado', 'POR PAGAR') == 'POR PAGAR' ? 'selected' : '' }}>POR PAGAR</option>
                ...
                <div class="form-actions">
                    <button type="button" class="btn btn-primary" onclick="showCustomConfirm()">
                        <i class="fas fa-save"></i> Guardar Subsidio
                    </button>
    ...
    <div id="custom-confirm-modal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.4);">
        <div style="background-color: #fefefe; margin: 15% auto; padding: 20px; border: 1px solid #888; width: 80%; max-width: 400px; border-radius: 8px;">
            <p><strong>¿Estás seguro de registrar este subsidio?</strong></p>
            <button onclick="submitForm()" style="background-color: #942044; color: white; padding: 8px 16px; border: none; border-radius: 4px; cursor: pointer; margin-right: 10px;">Sí, Registrar</button>
    ...
    <script>
        ...
        function submitForm() { 
            hideCustomConfirm(); 
            document.getElementById('create-subsidio-form').submit(); 
        }
